...
def PortfolioNLPModelCUDA_Construct(CovMat,             # 协方差矩阵, CuPy数组, shape=(N_Assets, N_Assets), 如果有暴露因子的情况，需要预先将暴露因子合并到协方差矩阵中
    ...
    return jl.PortfolioNLPModelCUDA_Construct(mean, cost, w0, Lambda_risk, cov_mat, n_con, x0, y0, w_lb, w_ub, lcon, ucon)
...
if __name__ == "__main__":
    ...
    Lcon = cp.full(N_con, 0.0, dtype=T)             # 约束条件的下限，这里我们只考虑权重之和最小为1的约束
    Ucon = cp.full(N_con, 1.0, dtype=T)             # 约束条件的上限，这里我们只考虑权重之和最大为1的约束 
    print("开始构建非线性规划的NLPModel模型")
    # 构建NLPModel模型，这里我们直接使用CuPy数组为参数
    julia_model = PortfolioNLPModelCUDA_Construct(Cov_Mat, Stocks_Mean_LR, Cost, W0, Lambda_risk, W_lb, W_ub, X0, N_con, Y0, Lcon, Ucon)
    ...
    julia_solver = jl.MadNLPGPU.MadNLPSolver(   julia_model,        # NLPModels的模型  
                                                ...
                                                max_iter = 50,    # 最大迭代次数，当迭代次数达到该值时，停止迭代。
