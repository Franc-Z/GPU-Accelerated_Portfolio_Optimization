...
JULIA_CODE_PATH = "/nvtest/GPU-Accelerated_Portfolio_Optimization/Mean-Risk(Barra)/mean_std_gpu_barra.jl"
# 检查Julia代码路径是否存在
if not exists(JULIA_CODE_PATH):
    raise FileNotFoundError(f"Julia代码路径 {JULIA_CODE_PATH} 不存在")

os.environ['JULIA_FLAGS'] = '--optimize=3'
# 在python中加载Julia代码
...
if jl.T == jl.Float32:
    ...
    TOL = 6e-6          # 求解精度容忍度(本身为python的float精度), 对于float32，可以适当放宽到5e-6
...
def PortfolioNLPModelCUDA_Construct(CovMat,             # 协方差矩阵, CuPy数组, shape=(N_Assets, N_Assets), 如果有暴露因子的情况，需要预先将暴露因子合并到协方差矩阵中
                                    ...
                                    Expo,               # 暴露因子矩阵, CuPy数组, shape=(N_Assets, N_Expo)
                                    Expo_T,
                                    Q,                  # Expo * CovMat * Expo_T, CuPy数组
    ...
    expo = CuPyArray2JuliaCuMatrix(Expo)
    expo_T = CuPyArray2JuliaCuMatrix(Expo_T)
    Q_julia = CuPyArray2JuliaCuMatrix(Q)
    print(f"行业类别个数={cls_count}")
    return jl.PortfolioNLPModelCUDA_Construct(mean, r_f, cost, cls, w0, Lambda_risk, cov_mat, n_con, x0, w_lb, w_ub, lcon, ucon, cls_count, expo, expo_T, Q_julia)

if __name__ == "__main__":
    Stocks_Mean_LR = cp.loadtxt('/nvtest/GPU-Accelerated_Portfolio_Optimization/stocks_return_original_2022.csv', dtype=T)  # 读取股票的平均收益率
    ...
    '''
    Cls = cp.loadtxt('/nvtest/GPU-Accelerated_Portfolio_Optimization/industry_labels.csv', dtype=cp.int64)
    assert N_Assets == len(Cls), "股票数量与分类向量的长度不一致"
    Cls, cats = cudf.factorize(Cls, sort=True)
    Cls = Cls + 1               # 由于Julia中的分类向量从1开始，所以我们需要将分类向量加1
    #print(cats)
    Cls = cp.array(Cls, dtype=cp.int64)

    Industry_Count_4_Constraint = len(cats)                            # 行业类别数量
    ...
    X0[-1] = Risk_min + 0.1
    N_con:int = 1 + 1 + Industry_Count_4_Constraint              # 约束条件的个数（必须是int64类型），这里我们考虑锥约束、权重之和为1的约束和行业权重约束
    ...
    Expo = cp.loadtxt('/nvtest/GPU-Accelerated_Portfolio_Optimization/expo_4558_41.csv', dtype=T, delimiter=",")
    Expo = cp.nan_to_num(Expo) 
    assert Expo.shape == (N_Assets, 41) ,"expo 矩阵的shape错误"
    Expo_T = cp.transpose(Expo)
    #Bias = cp.loadtxt('/nvtest/GPU-Accelerated_Portfolio_Optimization/bias_4558.csv', dtype=T)
    #Bias = cp.nan_to_num(Bias)
    Q = Expo@(Cov_Mat@Expo_T)
    Q = (Q + Q.T) * 0.5 + cp.eye(N_Assets) * 1e-8
    
    #cp.savetxt("/nvtest/industry_labels.csv", Cls)
    print("开始构建非线性规划的NLPModel模型")
    # 构建NLPModel模型，这里我们直接使用CuPy数组为参数
    julia_model = PortfolioNLPModelCUDA_Construct(Cov_Mat, 
                                                  Stocks_Mean_LR, 
                                                  Expo, Expo_T, 
                                                  Q, 
                                                  Cost, 
    ...
    julia_solver = jl.MadNLPGPU.MadNLPSolver(   julia_model,        # NLPModels的模型  
                                                ...
                                                max_iter = 300,     # 最大迭代次数，当迭代次数达到该值时，停止迭代。
                                                ...
                                                mu_superlinear_decrease_power = 1.75,            # 亚线性减少的幂次，用于控制亚线性减少的速度。
                                                #alpha_min_frac = 0.02,                           # 最小步长的比例，用于控制最小步长的大小。
    ...
    jl.update_parameters(   julia_model,                              # NLPModels的模型 
                            CuPyArray2JuliaCuVector(X0),                    # 上一次求解的权重向量结果，作为下一次求解的初始值
    ...
    """
    start_time = time()
    julia_results = jl.MadNLP.solve_b(julia_solver)
    ...
    x_result = JuliaCuVector2CuPyArray(julia_results.solution)
    x_var = x_result[:-1]
    print(cp.flip(cp.partition(x_var, -TopK)[-TopK:]))
    print(f'平均回报率 = {x_var @ Stocks_Mean_LR}')
    print(f"Risk = {x_result[-1]}")
    #print(f"risk = {sqrt(x_var.T@(Expo@(Cov_Mat@(Expo.T@x_var))))}")
