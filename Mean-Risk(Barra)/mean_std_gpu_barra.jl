...
import MadNLPGPU
import CUDA
import LinearAlgebra: isposdef, cholcopy, issymmetric, norm_sqr, I
import PythonCall
...
mutable struct PortfolioNLPModelCUDA{
    ...
    V_Buffer::VT                    # 存放[4558*1]向量的缓存
    v_buffer::VT                    # 存放[41*1]向量的缓存
    Σ::MT                           # 协方差矩阵Σ
    #L::MT                          # L矩阵每次求解需要更新
    U::MT                           # 对协方差矩阵进行LU分解的U矩阵，U矩阵每次求解需要更新
    Expo::MT                        # 存放Expo矩阵的缓存 [4558 * 41]
    Expo_T::MT                      # 存放Expo转置的矩阵的缓存 [41 x 4558]
    Q::MT                           # 存放Q矩阵的缓存 [4558 x 4558]
...
function PortfolioNLPModelCUDA_Construct(
    ...
    return PortfolioNLPModelCUDA(meta, Counters(), u_, cost_, cls_, w0_, V_Buffer_, v_buffer_, Σ_, U_, expo_, expo_T_, Q_, r_f, delta, cls_count_)
end
...
function NLPModels.obj(nlp::PortfolioNLPModelCUDA, x::AbstractVector{T}) where T
    ...
    t = x[end]              # 最后一个值为t，用于二阶锥约束，sqrt(w'Σw) - t <=0，
...
function set_Jacobian_by_class_kernel!(cls, J, n_con, n_assets, start_idx)
    idx = (CUDA.blockIdx().x - 1) * CUDA.blockDim().x + CUDA.threadIdx().x
    if idx <= n_assets
        cls_idx = cls[idx] + start_idx - 1
        if start_idx <= cls_idx <= n_con
            J[cls_idx, idx] = 1.0
        end
    end   
    return nothing
...
function NLPModels.cons!(nlp::PortfolioNLPModelCUDA, x::AbstractVector{T}, c::AbstractVector{T}) where T
    ...
    # println("con counter = ", nlp.counters.neval_cons)
    n = nlp.meta.nvar - 1
    x_var = x[1:n]
    t = x[end]
    # CUDA.fill!(c, zero(T))
    # 在 GPU 上计算
    #println(x_var)
    CUDA.CUBLAS.mul!(nlp.v_buffer, nlp.Expo_T, x_var)
    CUDA.CUBLAS.mul!(nlp.v_buffer, nlp.U, nlp.v_buffer)
    norm_Ux_sqr = norm_sqr(nlp.v_buffer)
    #println("t =", sqrt(norm_Ux_sqr))
    #println("norm =", sqrt(x_var'*nlp.Expo*nlp.Σ*nlp.Expo_T*x_var))
...
function MadNLP.jac_dense!(nlp::PortfolioNLPModelCUDA, x::AbstractVector{T}, J::AbstractMatrix{T}) where T
    ...
    CUDA.CUBLAS.mul!(nlp.v_buffer, nlp.Expo_T, x_var)
    CUDA.CUBLAS.mul!(nlp.v_buffer, nlp.Σ, nlp.v_buffer)
    CUDA.CUBLAS.mul!(nlp.V_Buffer, nlp.Expo, nlp.v_buffer)
    
    #CUDA.CUBLAS.mul!(nlp.V_Buffer, nlp.Q, x_var)
    #println(length(nlp.V_Buffer))
