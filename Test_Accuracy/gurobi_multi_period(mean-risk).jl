...
#set_optimizer_attribute(model, "MIPGap", 1e-6)            # 设置相对最优间隙为 0.01%
#set_optimizer_attribute(model, "MIPGapAbs", 1e-6)         # 设置绝对最优间隙
set_optimizer_attribute(model, "Threads", 8)              # 使用 4 个线程进行并行计算
...
@constraint(model, [t=1:T], [risk[t]; cov_U * y[:,t]; D_sqrt .* x[:,t]] in MathOptInterface.SecondOrderCone(soc_dim))
...
if status != MathOptInterface.OPTIMAL && status != MathOptInterface.ALMOST_OPTIMAL
